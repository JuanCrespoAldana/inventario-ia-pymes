...
class TestRecuperarContrasena:
    ...
    async def test_envia_correo_solo_si_el_usuario_existe(self, client, empresa_configurada):
        ...
        with patch("app.services.auth_service.enviar_email_recuperacion", new_callable=AsyncMock) as mock_enviar:
    ...
    async def test_flujo_completo_de_restablecer_contrasena(self, client, empresa_configurada):
        ...
        with patch("app.services.auth_service.enviar_email_recuperacion", new_callable=AsyncMock) as mock_enviar:
            await client.post("/api/auth/forgot-password", json={"email": USUARIO_VALIDO["email"]})
            enlace = mock_enviar.call_args.args[2]
            token = enlace.split("token=")[1]

        nueva_password = "NuevaClaveSegura456!"
        respuesta = await client.post(
            "/api/auth/reset-password", json={"token": token, "nueva_password": nueva_password}
        )
        assert respuesta.status_code == 200
    ...
    async def test_rechaza_token_invalido(self, client, empresa_configurada):
        respuesta = await client.post(
            "/api/auth/reset-password",
            json={"token": "token-que-no-existe", "nueva_password": "NuevaClaveSegura456!"},
        )
